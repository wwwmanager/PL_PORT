## 17.01.2026.16:50 — Исправление отображения формулы топлива при проверке

**Type:** fix  
**Zone:** waybills | ui

**Context/problem:**  
В модальном окне проверки ПЛ (`WaybillCheckModal`) в строке расшифровки формулы остатка топлива заправленное топливо отображалось со знаком минус (`-`), хотя математически оно прибавлялось. Это вводило пользователей в заблуждение, несмотря на верный итоговый результат.

**What was done:**
- Исправлен шаблон строки вывода формулы: заменен знак `-` на `+` перед значением заправки (`fuelFilled`).

**How verified:**
- Code review.

**Files:**
- [MODIFY] `components/waybills/WaybillCheckModal.tsx`

---

## 17.01.2026.16:00 — Исправление сохранения фильтров журнала

**Type:** fix  
**Zone:** waybills | ui

**Context/problem:**  
Настройки фильтров (дата, статус, ТС) сохранялись локально (`localhost`), но сбрасывались при перезагрузке страницы в продакшене (GitHub Pages). Вероятная причина — невалидные данные в `localStorage` от старых версий или конфликт кэша.

**What was done:**
- Изменен ключ хранения в `localStorage` на `waybillList_filters_v2` (принудительный сброс старых настроек).
- Усилена валидация при чтении сохраненных фильтров (безопасный парсинг JSON, проверка на объект).
- Добавлено логирование ошибок сохранения/чтения в консоль.
