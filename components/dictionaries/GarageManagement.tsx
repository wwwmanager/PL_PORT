...
import { DataTable, Column as DTColumn } from '../shared/DataTable';
...
const StockItemList = () => {
    ...
    const columnsConfig = useMemo<DTColumn<GarageStockItem>[]>(() => [
    ...
    const { rows, sortColumn, sortDirection, handleSort, filters, handleFilterChange } = useTable<GarageStockItem>(items, [
...
const AutoTopUpScheduleList = () => {
    ...
    // Enriched data for table display
    const enrichedData = useMemo(() => {
        return schedules.map(sch => {
            ...
            return {
                ...sch,
                driverName: driver?.shortName || 'Неизвестный водитель',
                itemName: item?.name || 'Неизвестный товар',
                itemUnit: item?.unit || 'ед.',
                frequencyLabel: sch.frequency === 'monthly' ? 'Ежемесячно' : 'Ежеквартально',
                statusLabel: sch.isActive ? 'Активно' : 'Отключено',
            };
        });
    }, [schedules, employees, stockItems]);

    type EnrichedSchedule = typeof enrichedData[0];

    const columnsConfig: DTColumn<EnrichedSchedule>[] = useMemo(() => [
        { key: 'driverName', label: 'Водитель', sortable: true },
        { key: 'itemName', label: 'Номенклатура', sortable: true },
        {
            key: 'quantity',
            label: 'Кол-во',
            sortable: true,
            render: (r) => <span className="font-semibold">{r.quantity} {r.itemUnit}</span>
        },
        { key: 'frequencyLabel', label: 'Периодичность', sortable: true },
        {
            key: 'isActive',
            label: 'Статус',
            sortable: true,
            render: (r) => (
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${r.isActive ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-200'}`}>
                    {r.statusLabel}
                </span>
            )
        },
        {
            key: 'lastExecutedAt',
            label: 'Посл. выполнение',
            sortable: true,
            render: (r) => r.lastExecutedAt ? new Date(r.lastExecutedAt).toLocaleDateString('ru-RU') : '—'
        },
    ], []);

    const { rows, sortColumn, sortDirection, handleSort, filters, handleFilterChange } = useTable<EnrichedSchedule>(enrichedData, [
        { key: 'driverName', label: 'Водитель' },
        { key: 'itemName', label: 'Номенклатура' },
        { key: 'quantity', label: 'Кол-во' },
        { key: 'frequencyLabel', label: 'Периодичность' },
        { key: 'isActive', label: 'Статус' },
        { key: 'lastExecutedAt', label: 'Посл. выполнение' },
    ]);

    const handleEdit = (item: EnrichedSchedule) => {
        const original = schedules.find(s => s.id === item.id);
        if (original) {
            reset(original);
            setCurrentSchedule(original);
        }
    };
    ...
    const handleDeleteRequest = (item: EnrichedSchedule) => {
        const original = schedules.find(s => s.id === item.id);
        if (original) setDeleteModal(original);
    };
        ...
        <div>
            ...
            <DataTable
                ...
                actions={[
                    ...
                    {
                        icon: <TrashIcon className="h-4 w-4" />,
                        onClick: (item) => handleDeleteRequest(item),
                        title: "Удалить",
                        className: "text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 p-1.5 rounded"
                    }
                ]}
            ...
            {rows.length === 0 && <p className="text-center text-gray-500 py-8 mt-4">Нет активных правил автопополнения.</p>}
