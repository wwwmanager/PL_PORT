...
const BatchList: React.FC<{ refreshBlanks: () => void }> = ({ refreshBlanks }) => {
    ...
    const [initialIssueState, setInitialIssueState] = useState({ driverId: '', range: { start: 0, end: 0 } });
    ...
    const openIssueModal = (batch: WaybillBlankBatch) => {
        ...
        setIssueRange(range);
        setDriverToIssue('');
        setInitialIssueState({ driverId: '', range });
    };
    ...
    const isIssueDirty = useMemo(() => {
        return driverToIssue !== initialIssueState.driverId || 
               issueRange.start !== initialIssueState.range.start || 
               issueRange.end !== initialIssueState.range.end;
    }, [driverToIssue, issueRange, initialIssueState]);
    ...
    return (
        <div>
            ...
            <Modal isOpen={!!issueModalBatch} onClose={() => setIssueModalBatch(null)} isDirty={isIssueDirty} title={`Выдать пачку ${issueModalBatch?.series}`} footer={<><button onClick={() => setIssueModalBatch(null)}>Отмена</button><button onClick={handleIssue}>Выдать</button></>}>
...
const BulkSpoilModal = ({isOpen, onClose, selection, onDone}: {isOpen: boolean, onClose: ()=>void, selection: SelectionState, onDone: ()=>void}) => {
    ...
    const { currentUser, rolePolicies } = useAuth();
    const { showToast } = useToast();
    
    const userPolicies = currentUser?.role ? rolePolicies[currentUser.role] : [];
    const allAbilities = useMemo(() => new Set([...userPolicies, ...(currentUser?.extraCaps || [])]), [currentUser, userPolicies]);
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            const result = await bulkSpoilBlanks(params, { abilities: allAbilities, actorId: currentUser.id, deviceId: 'webapp' });
